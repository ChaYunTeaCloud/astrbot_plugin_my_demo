...
HandoffTool 触发的是一个全新的、独立的 Agent 对话循环，功能更强大。
...
> 更多关于 Tool 机制（FunctionTool、HandoffTool 等）的内容，请参见 [Tool 机制](./Tool%20机制.md)

#### Agent（智能体定义）

**文件**：../.venv/Lib/site-packages/astrbot/core/agent/agent.py
...
**文件**：../.venv/Lib/site-packages/astrbot/core/agent/handoff.py
...
> 更多关于 Persona 机制（persona_id、人格设定配置）的内容，请参见 [Persona 机制](./Persona%20机制.md)
...
装饰器在 ../.venv/Lib/site-packages/astrbot/core/star/register/star_handler.py#L695-725 中实现：
...
- `../.venv/Lib/site-packages/astrbot/core/pipeline/process_stage/method/agent_sub_stages/internal.py#L269`
...
为了解决上述问题，一个直接思路是在 `on_llm_request` 钩子中，手动将所有 `HandoffTool` 对象注入到每个 SubAgent 的 `agent.tools` 属性中。
...
> 更多关于 Skill 机制（Skill 工具、Skill 管理）的内容，请参见 [Skill 机制](./Skill%20机制.md)
...
`FunctionToolManager`（../.venv/Lib/site-packages/astrbot/core/provider/func_tool_manager.py#L287-L292）维护了两个独立的列表：
...
内置工具按模块组织（../.venv/Lib/site-packages/astrbot/core/tools/registry.py#L12-L18）：
...
> 更多关于 Tool 机制的内容，请参见 [Tool 机制](./Tool%20机制.md)
>
> 更多关于 Persona 机制的内容，请参见 [Persona 机制](./Persona%20机制.md)
...
## 十、SubAgentOrchestrator 类详解

`SubAgentOrchestrator` 是负责从配置加载子 Agent 定义并创建 `HandoffTool` 列表的编排器。它本身是轻量级的——不执行 Agent，真正的执行发生在 `FunctionToolExecutor` 中的 `HandoffTool` 调用环节。

以下是 `SubAgentOrchestrator` 的完整源码与逐行解析：
