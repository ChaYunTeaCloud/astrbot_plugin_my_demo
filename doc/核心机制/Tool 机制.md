...
因为 `event` 是框架在调用 handler 时自动从上下文中提取并注入的（见 `../.venv/Lib/site-packages/astrbot/core/provider/func_tool_manager.py#L254-L255`）：
...
**原因**：框架在插件加载后会执行 handler 的重新绑定逻辑（见 ../.venv/Lib/site-packages/astrbot/core/star/star_manager.py#L1273-1298）。它会检查 `handler.__module__` 是否属于当前插件模块，如果是，则用 `functools.partial(handler, star_cls)` 把插件实例绑定为第一个参数（self）。
...
```python
# ../.venv/Lib/site-packages/astrbot/core/star/star_manager.py#L1276-1298
if ft.handler and (
...
```python
# ../.venv/Lib/site-packages/astrbot/core/agent/run_context.py#L22 - 预定义了无上下文的变体
...
```python
# ../.venv/Lib/site-packages/astrbot/core/star/context.py#L312 - tool_loop_agent 中
...
```python
# ../.venv/Lib/site-packages/astrbot/core/astr_agent_context.py#L21
...
```python
# ../.venv/Lib/site-packages/astrbot/core/star/context.py#L287
...
```python
# ../.venv/Lib/site-packages/astrbot/core/core_lifecycle.py#L227
...
**重要：LLM 传参不看顺序，按名称匹配**。从 `../.venv/Lib/site-packages/astrbot/core/provider/func_tool_manager.py#L255` 可以看到，handler 调用时用的是 `handler(event, **kwargs)`，`**kwargs` 是 LLM 传来的命名参数字典，Python 按参数名匹配，与 `properties` 中的定义顺序无关。
...
这就是 SubAgent 工具集缺失问题的根本原因（详见 [SubAgent 机制.md 第五章](./SubAgent%20机制.md)）。
...
**`computer_tools` 模块**（../.venv/Lib/site-packages/astrbot/core/tools/computer_tools/）：
