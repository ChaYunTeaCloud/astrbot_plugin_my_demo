...
- **Command（被调）**：AstrBot 识别到 `/xxx`，主动来找你：有没有人能处理？有就交给你。
  - 比如：
    - 我用 @filter.command(command_name="hello") 注册了一个 handler。
...
只有符合条件的才会被调用 。比如 @filter.command(command_name="hello") 只有用户发送 "hello" 命令时才会触发； @filter.on_llm_request() 只有在 LLM 请求发出时才会触发，不是每条消息都会走。
...
| 触发位置 | 代码 |
|---------|------|
| ../.venv/Lib/site-packages/astrbot/core/core_lifecycle.py#L367-L376 | `handler.handler()` — **无额外参数** |
...
| 触发位置 | 代码 |
|---------|------|
| ../.venv/Lib/site-packages/astrbot/core/pipeline/respond/stage.py#L322 | `call_event_hook(event, EventType.OnAfterMessageSentEvent)` — 只有 event |
...
##### 13. `OnPluginLoadedEvent` / `OnPluginUnloadedEvent` — 传 metadata / star_metadata

| 触发位置 | 代码 |
|---------|------|
| ../.venv/Lib/site-packages/astrbot/core/star/star_manager.py#L1421-L1429 | `await handler.handler(metadata)` — 传入插件元数据 |
| ../.venv/Lib/site-packages/astrbot/core/star/star_manager.py#L2044-L2054 | `await handler.handler(star_metadata)` — 传入插件元数据 |

##### 14. `AdapterMessageEvent` — 通过 `call_handler` 调用

| 触发位置 | 代码 |
|---------|------|
| ../.venv/Lib/site-packages/astrbot/core/pipeline/process_stage/method/star_request.py#L48 | 真正通过 `call_handler(event, handler.handler, **params)` 调用（waking_check/stage.py 只负责匹配/过滤 Handler），参数由指令解析决定 |
...
文件位置：../.venv/Lib/site-packages/astrbot/core/star/star_handler.py#L219-L241
...
14 个重载中，11 个返回类型为 Awaitable-only，2 个（`AdapterMessageEvent`、`OnCallingFuncToolEvent`）为 `Awaitable[Any] | AsyncGenerator[Any]`，另有 1 个兜底 `EventType` 重载（返回 `Awaitable[Any] | AsyncGenerator[Any]`）——但这也只是对这两类事件的 handler **理论上**可以是异步生成器的声明，并非方法本身能保证的。
...
3. **增加维护成本**：每新增一个 `EventType` 枚举值，都要手动添加对应的 `@overload` 定义，否则 TypeScript/pyright 会报类型错误。当前 AstrBot 有 16 个事件类型，14 个重载，已经出现了 `OnWaitingLLMRequestEvent`、`OnUsingLLMToolEvent` 和 `OnLLMToolRespondEvent` 三个**没有重载**的情况——调用这些事件时只能走兜底的宽泛类型。
...
| `get_sender_id()` / `get_sender_name()` | 获取发送者信息 |
| `get_group()` | 获取群信息 |
...
```python
# ../.venv/Lib/site-packages/aiocqhttp/api.py 第 27-30 行
