...
star_handlers_registry 是指向 StarHandlerRegistry 类的一个实例的全局变量吗？是的。定义位置：../.venv/Lib/site-packages/astrbot/core/star/star_handler.py#L216

StarHandlerRegistry 类的定义：../.venv/Lib/site-packages/astrbot/core/star/star_handler.py
从它的实现中可以看到它根据 EventType 来分类存储 Handler，重载了 get_handlers_by_event_type 方法。根据不同的 EventType，返回不同的 Handler 列表。

从 call_event_hook 的实现（../.venv/Lib/site-packages/astrbot/core/pipeline/context_utils.py #L78-112）也可以看到，它根据 hook_type 来调用不同的 Handler。
...
1. Handler 注册与元数据定义
   - ../.venv/Lib/site-packages/astrbot/core/star/star_handler.py
   - 包含： StarHandlerRegistry （注册表）、 StarHandlerMetadata （元数据）、 EventType （事件类型枚举）
2. 装饰器实现（@filter.xxx）
   - ../.venv/Lib/site-packages/astrbot/core/star/register/star_handler.py
   - 包含： register_command 、 register_on_llm_request 等所有装饰器函数
3. Handler 实际调用逻辑
   - ../.venv/Lib/site-packages/astrbot/core/pipeline/context_utils.py
   - 包含： call_handler() （执行单个 handler）、 call_event_hook() （执行事件钩子）
4. 命令/正则过滤器
   - ../.venv/Lib/site-packages/astrbot/core/star/filter/command.py — CommandFilter
   - ../.venv/Lib/site-packages/astrbot/core/star/filter/regex.py — RegexFilter
...
位置：../.venv/Lib/site-packages/astrbot/core/pipeline/context_utils.py #L78-112
...
位置：../.venv/Lib/site-packages/astrbot/core/pipeline/context_utils.py #L12-76
...
事件触发阶段（框架内部）：
  ../.venv/Lib/site-packages/astrbot/core/pipeline/process_stage/method/agent_sub_stages/third_party.py#L335:
...
位置：../.venv/Lib/site-packages/astrbot/core/pipeline/context_utils.py#L78-L112
...
| 触发位置 | 代码 |
|---------|------|
| ../.venv/Lib/site-packages/astrbot/core/core_lifecycle.py#L352-L361 | `handler.handler()` — **只传 event（通过 call_event_hook 时无额外参数）** |

#### 2. `OnPlatformLoadedEvent` — 无参数

| 触发位置 | 代码 |
|---------|------|
| ../.venv/Lib/site-packages/astrbot/core/platform/manager.py#L228-L236 | `handler.handler()` — 同上，无额外参数 |

#### 3. `OnWaitingLLMRequestEvent` — 无额外参数

| 触发位置 | 代码 |
|---------|------|
| ../.venv/Lib/site-packages/astrbot/core/pipeline/process_stage/method/agent_sub_stages/internal.py#L217 | `call_event_hook(event, EventType.OnWaitingLLMRequestEvent)` — 只有 `event` |

#### 4. `OnLLMRequestEvent` — 传 `req`（ProviderRequest）

| 触发位置 | 代码 |
|---------|------|
| ../.venv/Lib/site-packages/astrbot/core/pipeline/process_stage/method/agent_sub_stages/internal.py#L269 | `call_event_hook(event, EventType.OnLLMRequestEvent, req)` |
| ../.venv/Lib/site-packages/astrbot/core/pipeline/process_stage/method/agent_sub_stages/third_party.py#L335 | `call_event_hook(event, EventType.OnLLMRequestEvent, req)` |
...
| 触发位置 | 代码 |
|---------|------|
| ../.venv/Lib/site-packages/astrbot/core/astr_agent_hooks.py#L31-L35 | `call_event_hook(event, EventType.OnLLMResponseEvent, llm_response)` |
...
| 触发位置 | 代码 |
|---------|------|
| ../.venv/Lib/site-packages/astrbot/core/astr_agent_hooks.py#L17-L21 | `call_event_hook(event, EventType.OnAgentBeginEvent, run_context)` |
...
| 触发位置 | 代码 |
|---------|------|
| ../.venv/Lib/site-packages/astrbot/core/astr_agent_hooks.py#L36-L41 | `call_event_hook(event, EventType.OnAgentDoneEvent, run_context, llm_response)` |
...
| 触发位置 | 代码 |
|---------|------|
| ../.venv/Lib/site-packages/astrbot/core/astr_agent_hooks.py#L49-L54 | `call_event_hook(event, EventType.OnUsingLLMToolEvent, tool, tool_args)` |
...
| 触发位置 | 代码 |
|---------|------|
| ../.venv/Lib/site-packages/astrbot/core/astr_agent_hooks.py#L64-L70 | `call_event_hook(event, EventType.OnLLMToolRespondEvent, tool, tool_args, tool_result)` |
...
| 触发位置 | 代码 |
|---------|------|
| ../.venv/Lib/site-packages/astrbot/core/pipeline/result_decorate/stage.py#L159-L162 | 直接遍历 `star_handlers_registry.get_handlers_by_event_type(...)` 后 `handler.handler(event)` — 只传 event |

#### 11. `OnAfterMessageSentEvent` — 无额外参数

| 触发位置 | 代码 |
|---------|------|
| ../.venv/Lib/site-packages/astrbot/core/pipeline/respond/stage.py#L321 | `call_event_hook(event, EventType.OnAfterMessageSentEvent)` — 只有 event |

#### 12. `OnPluginErrorEvent` — 传 4 个参数

| 触发位置 | 代码 |
|---------|------|
| ../.venv/Lib/site-packages/astrbot/core/pipeline/process_stage/method/star_request.py#L59-L66 | `call_event_hook(event, EventType.OnPluginErrorEvent, md.name, handler.handler_name, e, traceback_text)` |
...
| 触发位置 | 代码 |
|---------|------|
| ../.venv/Lib/site-packages/astrbot/core/star/star_manager.py#L1421-L1424 | `handler.handler()` — 只传 event |
| ../.venv/Lib/site-packages/astrbot/core/star/star_manager.py#L2008-L2011 | 同上 |

#### 14. `AdapterMessageEvent` — 通过 `call_handler` 调用

| 触发位置 | 代码 |
|---------|------|
| ../.venv/Lib/site-packages/astrbot/core/pipeline/waking_check/stage.py#L163-L165 | 通过 `call_handler(event, handler.handler, **params)` 调用，参数由指令解析决定 |

#### 15. `OnCallingFuncToolEvent` — 特殊处理

此事件类型**不通过 `call_event_hook` 触发**。它在 ../.venv/Lib/site-packages/astrbot/core/star/register/star_handler.py#L669 注册后，handler 被存入 `FunctionToolManager`，由 Agent 工具执行器直接调用，参数为 LLM 返回的工具调用参数。

---

### `EventType` 枚举定义

文件位置：../.venv/Lib/site-packages/astrbot/core/star/star_handler.py#L218-L240
...
这种设计的优点是**简单直接**——`call_event_hook` 不需要了解任何 `EventType` 的语义，新增事件类型只需在调用方传对参数即可。缺点是**缺乏运行时校验**——如果 Handler 的参数签名与触发点不一致，只会在 `call_handler` 中被 `TypeError` 捕获并记录日志（见 ../.venv/Lib/site-packages/astrbot/core/pipeline/context_utils.py#L38-L42），不会阻止事件传播。
...
../.venv/Lib/site-packages/astrbot/core/star/star_handler.py#L32-L148 有 **14 个 `@overload` 定义**（13 个具体事件类型 + 1 个兜底），而实际实现只有 [L150-L186](../.venv/Lib/site-packages/astrbot/core/star/star_handler.py#L150-L186) 一个。
...
filePath: ../.venv/Lib/site-packages/astrbot/core/star/register/star_handler.py
...
平台差异控制在 `api.event.filter` 中。

---

## 一、`support_platforms` 的作用

`support_platforms` 在 `metadata.yaml` 中声明，**只用于 WebUI 展示**，不影响代码执行：
...
它的作用是在插件管理页面告诉用户"这个插件支持哪些平台"。

---

## 二、真正控制平台差异的 API

在 `api.event.filter` 中有 `platform_adapter_type` 装饰器，用于**过滤特定平台的消息**：
...
async def qq_command(self, event):
    yield event.make_result().message("只有QQ用户能看到")

# 方式二：使用枚举
@filter.platform_adapter_type(PlatformAdapterType.DISCORD | PlatformAdapterType.SLACK)
@filter.command("chat_only")
async def chat_command(self, event):
    yield event.make_result().message("只有Discord/Slack用户能看到")

# 方式三：排除特定平台
@filter.platform_adapter_type(PlatformAdapterType.ALL ^ PlatformAdapterType.WEBCHAT)
@filter.command("not_webchat")
async def not_webchat_command(self, event):
    yield event.make_result().message("WebChat用户看不到")
```bash

此外，在 ../.venv/Lib/site-packages/astrbot/core/platform/sources 目录下的每个平台的事件类中可以看到，部分平台的事件类继承了 `AstrMessageEvent`，并添加了平台特定的方法。有些方法在其它平台的事件类中是不存在或不适配的。如：../.venv/Lib/site-packages/astrbot/core/platform/astr_message_event.py 第 503 行定义的 get_group 方法，其有说明只支持 aiocqhttp 平台。

---

## 三、PlatformAdapterType 枚举

**文件**：../.venv/Lib/site-packages/astrbot/core/star/filter/platform_adapter_type.py#L9
...
在插件代码中，可以通过 `event` 获取当前平台信息：

```python
@filter.command("info")
async def get_platform_info(self, event):
    # 获取平台名称（字符串）
    platform_name = event.get_platform_name()  # "aiocqhttp", "discord" 等
    
    # 获取平台元数据
    platform_id = event.platform_meta.id
    platform_desc = event.platform_meta.description
    
    yield event.make_result().message(f"当前平台: {platform_name}")
```json
...
| 用途 | 方式 |
|------|------|
| 在 WebUI 展示支持的平台 | `metadata.yaml` 中声明 `support_platforms` |
| 过滤特定平台的消息 | `@filter.platform_adapter_type()` 装饰器 |
| 在代码中判断平台 | `event.get_platform_name()` |

所以 `support_platforms` 和 `platform_adapter_type` 是配套使用的：前者声明给用户看，后者实际控制代码行为。

## 实际应用场景

大部分插件不需要平台差异控制。但在某些场景下它是有用的：
1. 平台特有消息组件（如QQ特有的戳一戳、QQ特有的表情）
2. 平台特定功能调用）如获取群成员、设置管理员）
3. 不同平台的消息展示优化（某些平台对消息格式有特殊要求，如 webchat 适合展示富文本，而其它平台只能用纯文本）
4. 平台特定的限制处理（不通平台有不同的限制，如消息长度、文件大小等）
5. 平台特定的认证方式（某些功能需要平台特定的认证，如企业微信的审批流程）

大部分插件确实不需要平台差异控制 。但当你的插件：

- 使用了平台特有的消息组件（ Poke 、 Face 、 Share 等）
- 需要调用平台特定的 API（群管理、审批等）
- 需要针对不同平台做消息适配
这时 platform_adapter_type 就有用了。如果你的插件是通用的（只处理文本、图片等通用消息），确实用不到它。

# 事件类的继承层次

Q:我要怎么知道 event 的哪些方法是只有特定平台能用的呢？

A:好问题！这涉及到 AstrBot 的事件继承体系。让我分析一下：
...
# 管线会将 AstrBotMessage 包装为特定平台实现的继承类

AstrBot的管线在接收到消息时会根据平台判断，然后将 AstrBotMessage 包装为特定平台实现的继承类

注：平台适配器本身就是已经继承了 `Platform` 类的实例，如 `AiocqhttpAdapter` 它本身已经重载了 `create_event` 方法，所以会返回 `AiocqhttpMessageEvent` 类的实例。
...
**文件**：../.venv/Lib/site-packages/astrbot/core/platform/platform.py
...
**QQ (aiocqhttp)**：../.venv/Lib/site-packages/astrbot/core/platform/sources/aiocqhttp/aiocqhttp_platform_adapter.py
...
**Discord**：../.venv/Lib/site-packages/astrbot/core/platform/sources/discord/discord_platform_adapter.py
...
**Telegram**：../.venv/Lib/site-packages/astrbot/core/platform/sources/telegram/tg_adapter.py
...
- 平台适配器是单例的，在 AstrBot 启动时创建，整个生命周期只创建一次。创建流程：
  - AstrBot 启动时，读取配置文件中的平台配置
  - 通过 register_platform_adapter 装饰器找到对应的适配器类
  - 创建适配器实例，传入配置和事件队列
  - 调用 adapter.run() 启动平台（建立 WebSocket 连接等）
  - 之后所有消息都通过同一个适配器实例处理
在 ../.venv/Lib/site-packages/astrbot/core/platform/manager.py 应该有相关代码

平台适配器继承自 `Platform` 类，如 `AiocqhttpAdapter`。`AiocqhttpAdapter` 就是 OneBot V11 标准的消息平台适配器，支持反向 WebSockets。

# 访问平台原生API

通用 API 功能有限，只能获取基本信息。因此，为了功能丰富，可调用所有 OneBot v11 API 方法。

注意事项：
- 权限要求 ：获取群列表和好友列表等功能可能需要机器人有相应的权限
- 异步调用 ：所有 API 都是异步方法，需要使用 await
- 错误处理 ：建议添加 try-except 捕获可能的错误
- 平台检查 ：在调用前检查平台是否加载，避免 AttributeError

## 重点：获取对应平台框架的客户端实例

以 aiocqhttp 为例:
AiocqhttpAdapter 中有一个 self.bot 属性(类型为 CQHttp 的实例，定义在第55行)，它就是 aiocqhttp 的 CQHttp 客户端：
```python
self.bot = CQHttp(
    use_ws_reverse=True,
...
CQHttp 是 aiocqhttp 框架提供的，而不是 AstrBot 自己实现的。AstrBot 导入了它，所以可以直接使用。
通过这个 `bot` 实例，可以调用所有 OneBot v11 API 方法，如：
...
# 获取群成员列表
member_list = await bot.get_group_member_list(group_id=123456)
```bash
...
# 方式二：按 ID 获取（推荐）
qq_platform = self.context.get_platform_inst("aiocqhttp")
```bash
## 完整示例
...
class MyPlugin(Star):
    ...
    async def get_groups(self, event):
        # 获取 QQ 平台实例
        qq_platform = self.context.get_platform_inst("aiocqhttp")
        if not qq_platform:
            yield event.make_result().message("QQ 平台未加载")
            return
        
        # 获取 aiocqhttp 客户端
        bot = qq_platform.bot
        
        # 调用 OneBot v11 原生 API
        try:
            group_list = await bot.get_group_list()
            
            if not group_list:
                yield event.make_result().message("暂无群聊")
            else:
                msg = "\n".join([f"群号: {g['group_id']}, 名称: {g['group_name']}" for g in group_list])
                yield event.make_result().message(msg)
        
        except Exception as e:
            logger.error(f"获取群列表失败: {e}")
            yield event.make_result().message(f"获取群列表失败: {str(e)}")
    
    @filter.command("friends")
    async def get_friends(self, event):
        qq_platform = self.context.platform_manager.get_platform("aiocqhttp")
        if not qq_platform:
            yield event.make_result().message("QQ 平台未加载")
            return
        ...
        try:
            ...
            if not friend_list:
                yield event.make_result().message("暂无好友")
            else:
                msg = "\n".join([f"QQ: {f['user_id']}, 昵称: {f['nickname']}" for f in friend_list])
                yield event.make_result().message(msg)
        
        except Exception as e:
            logger.error(f"获取好友列表失败: {e}")
            yield event.make_result().message(f"获取好友列表失败: {str(e)}")
```bash


# 以 aiocqhttp 为例，如何通过 `bot` 对象调用 cqhttp 接口

详细说明 AstrBot 框架内部是如何使用 `bot` 对象调用 cqhttp 接口的：

## 一、`bot` 对象的传递链

```
AiocqhttpAdapter.__init__()
    ↓ 创建 CQHttp 实例
self.bot = CQHttp(...)

AiocqhttpAdapter.create_event(message)
    ↓ 将 bot 传给事件对象
return AiocqhttpMessageEvent(..., bot=self.bot)

AiocqhttpMessageEvent.__init__()
    ↓ 保存 bot 对象
self.bot = bot
```bash

**文件**：../.venv/Lib/site-packages/astrbot/core/platform/sources/aiocqhttp/aiocqhttp_platform_adapter.py#L55

---

## 二、框架内部调用 cqhttp 接口的方式

### 方式一：直接调用（如 `bot.send_group_msg()`）

**文件**：../.venv/Lib/site-packages/astrbot/core/platform/sources/aiocqhttp/aiocqhttp_message_event.py#L107
...
### 方式二：通过 `call_action` 调用（如 `bot.call_action("get_group_info")`）

**文件**：../.venv/Lib/site-packages/astrbot/core/platform/sources/aiocqhttp/aiocqhttp_message_event.py#L248
...
**文件**：../.venv/Lib/site-packages/astrbot/core/platform/sources/aiocqhttp/aiocqhttp_message_event.py#L167
...
因为 `bot.send_group_msg()` 会通过 `__getattr__` 转换为 `bot.call_action("send_group_msg", ...)`：
...
# 方式二：显式调用（底层实现）
await bot.call_action("send_group_msg", group_id=123, message="Hello")
...
以 `AiocqhttpMessageEvent.get_group()` 为例：
...
回到你的问题：**插件中如何获取群列表和好友列表？**

通过 `context.platform_manager.get_platform("aiocqhttp")` 获取平台实例，然后访问 `bot` 属性：
...
| 问题 | 答案 |
|------|------|
| `bot` 对象从哪来？ | `AiocqhttpAdapter.__init__()` 中创建的 `CQHttp` 实例 |
| 如何传递给事件？ | `create_event()` 时作为参数传给 `AiocqhttpMessageEvent` |
| 框架内部如何调用？ | `self.bot.call_action("api_name", **params)` 或 `self.bot.api_name(**params)` |
| 插件中如何调用？ | `context.platform_manager.get_platform("aiocqhttp").bot` |

**核心要点**：AstrBot 在平台适配器初始化时创建 `CQHttp` 实例，然后通过事件对象传递给各个处理方法。插件开发者可以通过 `platform_manager` 获取平台实例，然后直接使用 `bot` 对象调用任何 OneBot v11 API。

## 方式一语法糖的说明

这不是语法糖，而是 Python 的**魔术方法**（magic method）机制。具体是 `__getattr__` 方法。

**官方文档**：
- [Python 文档 - 特殊方法名称](https://docs.python.org/zh-cn/3/reference/datamodel.html#object.__getattr__)

`__getattr__` 的作用：当通过 `obj.attr` 访问一个**不存在的属性**时，Python 会自动调用 `obj.__getattr__("attr")`。

在 aiocqhttp 中的实现：

```python
# aiocqhttp/api.py 第 27-30 行
...
所以 `bot.send_group_msg(group_id=123)` 等价于 `bot.call_action("send_group_msg", group_id=123)`。

这是 aiocqhttp 库实现的，不是 AstrBot 实现的，也不是 Python 的语法糖，而是利用 Python 的 `__getattr__` 魔术方法实现的**动态方法分发**。
...
- `AiocqhttpAdapter`（../.venv/Lib/site-packages/astrbot/core/platform/sources/aiocqhttp/aiocqhttp_platform_adapter.py#L35
）：平台适配器，负责连接管理、消息接收，**内部创建并持有** `bot` 对象（第 55 行）

- `AiocqhttpMessageEvent`（../.venv/Lib/site-packages/astrbot/core/platform/sources/aiocqhttp/aiocqhttp_message_event.py#L22）：事件类，负责消息发送和平台特定操作，**通过构造函数接收** `bot` 对象（第 29 行）

所以调用 aiocqhttp API 的不是这两个类本身，而是它们持有的 `bot: CQHttp` 实例。这个实例在适配器创建，传递给事件对象，最终在插件中通过 `event.bot` 或 `platform.bot` 访问。
