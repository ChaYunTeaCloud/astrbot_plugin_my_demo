...
> 源码路径：
> - EventBus: `astrbot/core/event_bus.py`
> - PipelineScheduler: `astrbot/core/pipeline/scheduler.py`
> - Stage 基类: `astrbot/core/pipeline/stage.py`
> - Stage 顺序: `astrbot/core/pipeline/stage_order.py`
...
**文件**：astrbot/core/pipeline/stage.py
...
**文件**：astrbot/core/pipeline/context.py
...
按执行顺序排列，定义在 `astrbot/core/pipeline/stage_order.py`：

### 3.1 WakingCheckStage（唤醒检查）

**文件**：astrbot/core/pipeline/waking_check/stage.py
...
**文件**：astrbot/core/pipeline/whitelist_check/stage.py

**功能**：检查群聊/私聊是否在白名单中。

### 3.3 SessionStatusCheckStage（会话状态检查）

**文件**：astrbot/core/pipeline/session_status_check/stage.py

**功能**：检查会话是否启用。

### 3.4 RateLimitStage（频率限制）

**文件**：astrbot/core/pipeline/rate_limit_check/stage.py

**功能**：检查会话是否超过频率限制。

### 3.5 ContentSafetyCheckStage（内容安全）

**文件**：astrbot/core/pipeline/content_safety_check/stage.py

**功能**：检查消息内容是否安全，支持百度 AI 安全审核、关键词过滤等策略。

### 3.6 PreProcessStage（预处理）

**文件**：astrbot/core/pipeline/preprocess_stage/stage.py
...
**文件**：astrbot/core/pipeline/process_stage/stage.py

**功能**：执行插件 Handler + LLM 调用，包含两个子阶段：

**7a. StarRequestSubStage**（astrbot/core/pipeline/process_stage/method/star_request.py）：
...
**文件**：astrbot/core/pipeline/result_decorate/stage.py
...
**文件**：astrbot/core/pipeline/respond/stage.py
...
**文件**：astrbot/core/platform/sources/aiocqhttp/aiocqhttp_platform_adapter.py#L85
...
**文件**：astrbot/core/platform/sources/aiocqhttp/aiocqhttp_platform_adapter.py#L198
...
**文件**：astrbot/core/pipeline/context_utils.py#L12
